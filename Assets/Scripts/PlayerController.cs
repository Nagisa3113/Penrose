...
public class PlayerController : MonoBehaviour
{
    ...
    [Space] public List<Transform> finalPath = new List<Transform>();
    ...
    void ExploreCube(List<Transform> nextCubes, List<Transform> visitedCubes)
    {
        ...
        foreach (var path in current.GetComponent<Walkable>().possiblePaths)
        {
    ...
    void FollowPath()
    {
        ...
        for (int i = finalPath.Count - 1; i > 0; i--)
        {
            ...
            s.Append(transform.DOMove(finalPath[i].GetComponent<Walkable>().GetWalkPoint(), .2f * time)
                .SetEase(Ease.Linear));
        }
